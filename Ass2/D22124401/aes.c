...
unsigned char *expand_key(unsigned char *cipher_key) {
    ...
    key_expansion(cipher_key, round_keys);

    unsigned char *expanded_key = malloc(176 * sizeof(unsigned char));
    for (int i = 0; i < 11; ++i) {
        for (int j = 0; j < 4; ++j) {
            for (int k = 0; k < 4; ++k) {
                expanded_key[i * 16 + j * 4 + k] = round_keys[i][j][k];
            }
        }
    }
    return expanded_key;
...
void aes_encrypt(uint8_t input_block[16], uint8_t key[16], uint8_t output_block[16]) {
    ...
    // Convert input_block and key to 4x4 matrices
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
    ...
    // Key expansion
    uint8_t expanded_key[11][4][4];
    key_expansion(key_matrix, expanded_key);

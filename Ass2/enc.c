...
uint8_t sub_word(uint8_t word[4]) {
    for (int i = 0; i < 4; ++i) {
        word[i] = s_box[word[i]];
    }
}
...
void key_expansion(const uint8_t key[16], uint8_t round_keys[11][4][4]) {
    int i = 0;
    uint8_t temp[4];

    // Copy the input key to the round keys
    for (i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
    ...
    for (i = 1; i <= 10; ++i) {
        // Get the previous column from the round keys
        for (int j = 0; j < 4; ++j) {
            temp[j] = round_keys[i - 1][3][j];
        }

        // Perform key schedule core operation
        rot_word(temp);
        sub_word(temp);
        temp[0] ^= rcon[i];

        // XOR with the previous 4-byte block and store the new column in round keys
        for (int j = 0; j < 4; ++j) {
            for (int k = 0; k < 4; ++k) {
                round_keys[i][j][k] = round_keys[i - 1][j][k] ^ temp[k];
            }
            // Update temp with the new column
            for (int k = 0; k < 4; ++k) {
                temp[k] = round_keys[i][j][k];
            }
        }
    }
}

void aes_128_encrypt(const uint8_t input[16], const uint8_t key[16], uint8_t output[16]) {
    uint8_t state[4][4];
...
unsigned char encrypt_block(unsigned char plaintext, unsigned char key) {
    size_t len = sizeof(uchar_array) / sizeof(uchar_array[0]);
    uint8_t uint8_array[len];

    for (size_t i = 0; i < len; ++i) {
        uint8_array[i] = (uint8_t) uchar_array[i];
    }
...
unsigned char decrypt_block(unsigned char ciphertext, unsigned char key) {
    size_t len = sizeof(uint8_array) / sizeof(uint8_array[0]);
    unsigned char uchar_array[len];

    for (size_t i = 0; i < len; ++i) {
        uchar_array[i] = (unsigned char) uint8_array[i];
    }
